...
if(isset($_POST["admin_email_address"]))
{
	...
	if($total_row == 0)
	{
		$object->query = "
			...
			WHERE mechanic_email_address = :admin_email_address
		";
		$object->execute($data);
		...
		else
		{
			...
			{
				if($row["mechanic_status"] == 'Inactive')
				{
					$error = '<div class="alert alert-danger">Your account is Inactive, Contact Admin</div>';
				}
				else
				{
					if($_POST["admin_password"] == $row["mechanic_password"])
					{
						$_SESSION['admin_id'] = $row['mechanic_id'];
						$_SESSION['type'] = 'Mechanic';
						$url = $object->base_url . 'admin/mechanic_schedule.php';
					}
					else
					{
