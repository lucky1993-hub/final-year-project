...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'check_login')
	{
		...
		{
			echo 'dashboard.php? mechanic_id= 1';
		}
		else
		{
			echo 'login.php';
	...
	if($_POST['action'] == 'customer_register')
	{
		...
		else
		{
			...
			VALUES (:customer_email_address, :customer_password, :customer_name,:customer_address, :customer_phone_no, :customer_added_on, :customer_verification_code, :email_verify)
			";
	...
	if($_POST['action'] == 'fetch_schedule')
	{
		...
		$order_column = array('mechanic_table.mechanic_name', 'mechanic_table.mechanic_address', 'mechanic_table.mechanic_phone_no', 'mechanic_schedule_table.mechanic_schedule_date', 'mechanic_schedule_table.mechanic_schedule_day', 'mechanic_schedule_table.mechanic_schedule_start_time');
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			$search_query .= 'OR mechanic_table.mechanic_phone_no LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = $row["mechanic_phone_no"];
	...
	if($_POST['action'] == 'edit_profile')
	{
		$data = array(
			...
			':custome_phone_no'			=>	$_POST["customer_phone_no"]
		...
		custome_phone_no = :custome_phone_no
	...
	if($_POST['action'] == 'book_appointment')
	{
		...
		else
		{
			...
			(mechanic_id, customer_id, mechanic_schedule_id, appointment_number, reason_for_appointment, appointment_time, status) 
			VALUES (:mechanic_id, :customer_id, :mechanic_schedule_id, :appointment_number, :reason_for_appointment, :appointment_time, :status)
			";
