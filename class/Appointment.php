...
class Appointment
{
	public $base_url = 'http://localhost/src/auto/';
	public $connect;
	public $query;
	public $statement;
	public $now;
	...
	{
		$this->connect = new PDO("mysql:host=localhost;dbname=fleet", "root", "");
	...
	function get_total_today_appointment()
	{
		...
		WHERE mechanic_schedule_date = CURDATE() 
		";
		$this->execute();
	...
	function get_total_yesterday_appointment()
	{
		...
		INNER JOIN mechanic_schedule_table 
		ON mechanic_schedule_table.mechanic_schedule_id = appointment_table.mechanic_schedule_id 
		WHERE mechanic_schedule_date = CURDATE() - 1
		";
		$this->execute();
		return $this->row_count();
	}
	...
	function get_total_seven_day_appointment()
	{
		...
		INNER JOIN mechanic_schedule_table 
		ON mechanic_schedule_table.mechanic_schedule_id = appointment_table.mechanic_schedule_id 
		WHERE mechanic_schedule_date >= DATE(NOW()) - INTERVAL 7 DAY
		";
		$this->execute();
		return $this->row_count();
	}
